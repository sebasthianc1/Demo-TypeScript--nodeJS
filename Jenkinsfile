...
//      - bot-testing-artifactory-vdc-npmrc = a .npmrc file configured for artifactory
//      - bot-testing-vdc-art = user/password credential for artifactory
...
pipeline {
    ...
    environment {
        cacheKey = "cache-tests-node-demo"
    ...
    stages {
        stage("Initialize") {
            steps {
                withCredentials([file(credentialsId: 'bot-testing-artifactory-vdc-npmrc', variable: 'NPMRC')]) {
                    sh "cp \${NPMRC} .npmrc"
                }
            }
        }
            ...
            steps {
                runTests([
                    context: [
                        ...
                        artifactoryNpmCredentials: 'bot-testing-artifactory-vdc-npmrc',
                    ],
                    tests: [
                        [
                            type: 'e2e',
